...
const setDomElements = (
  ...
  locationZone.textContent = data.timezone;
  temperatureSpan.textContent = "F";
  // Calling the set icons
...
// @link https://cdnjs.com/libraries/skycons
const setIcons = (icon, iconID) => {
  console.log("icons", iconID);
  // We need to init the Skycons adding as params the color of the icon
  const skycons = new Skycons({
    color: "white"
  });
  console.log(skycons);
  // From API we get icon: something-xxx-xxx
  // we need to replace "-" with "_" to adapt to skycons
  // and to upper case as in Skycons are uppercase
  const currentIcon = icon.replace(/-/g, "_").toUpperCase();
  console.log(currentIcon);
  // We need to start the animation
  skycons.play();
  // We return the icon we need
  const test = skycons.set(iconID, Skycons[currentIcon]);
  return test;
};

// We change F to C as API shows only in F
const changeTemperatureDegree = (
  temperature,
  temperatureSection,
  temperatureSpan,
  temperatureDegree
) => {
  // Formula conversion to celsius
  let celsius = (temperature - 32) * (5 / 9);
  // On click on the temperature we change from F to C and vice versa
